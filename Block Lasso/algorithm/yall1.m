...
% solve the problem
% tic; 
[x1,Out] = yall1_solve(A, At, b1, x0, z0, opts, bmax);
%Out.time = toc;
...
function [x, Out] = yall1_solve(A,At,b,x0,z0,opts,bmax)
...
for iter = 1:maxit
    ...
    rd = Aty - z;
    x = x + (gamma*mu) * rd;
    Out.res(iter) = norm(x.*bmax - opts.x)/opts.normx;
...
function check_stopping
    res1 = norm(x.*bmax - xp.*bmax)/norm(x.*bmax);
    res2 = norm(A(x.*bmax)-b.*bmax);
    res = max(res1 , res2);
    Out.error(iter) = res;
    if res <= opts.tol
        stop = 1; return
